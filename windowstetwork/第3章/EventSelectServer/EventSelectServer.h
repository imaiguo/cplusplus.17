...
BOOL InsertSocketObj(PTHREAD_OBJ pThread, PSOCKET_OBJ pSocket)
{
	BOOL bRet = FALSE;
	::EnterCriticalSection(&pThread->cs);
	//WSA_MAXIMUM_WAIT_EVENTS=64路链接同时在线,每个线程可以处理64个
	int i=WSA_MAXIMUM_WAIT_EVENTS;
	...
	if(bRet)
	{
		::InterlockedIncrement(&g_nTatolConnections);					//让++成为原子操作
	...
	do
	{
		if(event.lNetworkEvents & FD_READ)			// 套节字可读
		{
			...
			//处理会话
				...
				char szText[256]={'\0'};
				int i=strlen(szText);
				//如果超过指定的255个字符，则分多次接受
				int nRecv = ::recv(pSocket->s, szText, 255, 0);
				if(nRecv > 0)				
				{
					szText[nRecv] = '\0';
					printf("接收到数据：%s \n", szText);
...
DWORD WINAPI ServerThread(LPVOID lpParam)
{
	// 取得本线程对象的指针，每个线程的只处理自己的socket组中的所有的socket的消息
	...
	while(TRUE)
	{
		...
		for(int i=nIndex; i<pThread->nSocketCount + 1; i++)
		{
			...
			else
			{
				...
				{
					// 查找对应的套节字对象指针，调用HandleIO处理网络事件，更具事件找到socket,并处理
